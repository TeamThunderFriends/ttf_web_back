...
public record UserRequest(
	...
	@Pattern(regexp = "^[a-z0-9]*$", message = "아이디는 영문 소문자 혹은 숫자로만 구성되어야 합니다.") String userId,

	@NotBlank(message = "비밀번호를 입력해주세요.")
	@Size(min = 8, max = 16, message = "비밀번호는 8~16자 영문 대문자, 소문자, 숫자로 구성되어야 합니다.")
	@Pattern(regexp = "^(?=.*[a-z])(?=.*[A-Z])(?=.*\\d)[a-zA-Z\\d]{8,16}$", message = "비밀번호는 영문 대문자, 소문자, 숫자가 포함되어야 합니다.") String userPassword) {
